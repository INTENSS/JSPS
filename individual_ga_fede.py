...
class Individual:    
    ...
    def map_masks_to_weights(self, neural_network: MLP):
        ...
        params = torch.numel(neural_network.fc1.weight) + \
                         torch.numel(neural_network.fc1.bias) + \
                         torch.numel(neural_network.fc2.weight) + \
                         torch.numel(neural_network.fc2.bias)
        for i in range(1, len(neural_network.mask)):
    ...
    def set_evolutionary_history(self, generation, tot_disconnectable_units):
        self.evolutionary_history.append((generation, self.get_feature_sparsity(), self.get_feature_disconnected_units(tot_disconnectable_units)))
    ...
    def mutation_polynomial_bounded(self, mutation_probability: float, eta: float, lower: float, upper: float):
        for i in range(len(self.genome_weights)):
                if torch.rand(1) < mutation_probability:
                    ...
                    if torch.isnan(self.genome_weights[i]):
                        print('NAN:  ', i)
        #return genome_weights
    ...
    def mutation_pruning(self, disconnection_probability: float, connection_probability: float):
